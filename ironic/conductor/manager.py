...
class ConductorManager(periodic_task.PeriodicTasks):
    ...
    @messaging.expected_exceptions(exception.NoFreeConductorWorker,
                                 ...
                                 exception.MissingParameterValue)
    def get_vendor_passthru(self, context, node_id, driver_method):
        """RPC method to encapsulate vendor action.

        Synchronously validate driver specific info or get driver status,
        and if successful, call vendor specific get method.
        ...
        """
        LOG.debug(_('RPC get_vendor_passthru for node %s')
                  % node_id)
        ...
        with task_manager.acquire(context, node_id, shared=True) as task:
            ...
            task.driver.vendor.validate(task, node, method=driver_method)
            return task.driver.vendor.vendor_passthru(task, node, method=driver_method)
